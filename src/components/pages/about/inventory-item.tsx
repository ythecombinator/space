...
function InventoryItemRoot({ children }: PropsWithChildren) {
  return <li className="group relative flex flex-col items-start">{children}</li>;
}

function InventoryItemLink({ children, ...props }: PropsWithChildren<LinkProps>) {
  return (
    <>
...
function InventoryItemTitle({ href, children }: PropsWithChildren<{ href: string }>) {
  return (
